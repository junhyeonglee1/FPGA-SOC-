...
void ssd1306_WriteCommand(uint8_t byte) {
	uint8_t buffer[2] = {0};
		...
		XIic_Send(XPAR_IIC_1_BASEADDR, OLED_SLAVE_ADDRESS, buffer, 2, XIIC_STOP);
}

// Send data
void ssd1306_WriteData(uint8_t* data_buffer, size_t buffer_size) {
//	if(buffer_size > 16) {
//		//xil_printf("oversize!!!\n");
//		return ;
//	}
	...
	uint8_t Buffer[128 + 1]={0};
		...
		XIic_Send(XPAR_IIC_1_BASEADDR, OLED_SLAVE_ADDRESS, Buffer, buffer_size+1 , XIIC_STOP);
}
...
//        for(uint8_t j = 0; j < 16;j++){
			ssd1306_WriteData(&SSD1306_Buffer[SSD1306_WIDTH * i], 128);
 //       }
